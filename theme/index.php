...
?>
  ...
  <section id="content-loop" class="py-2">
    <div class="container p-0">
        ...
        <div class="row">
            <h2 class="title_loop_homepage2 titre animated slideInRight"><?= get_theme_mod('title_loop_homepage2', 'Nos derniers articles'); ?></h2>
          </div>
    ...
    <?php
      ...
      $args = array(
        'posts_per_page' => get_theme_mod('num_loop_homepage2', 24),
        'ignore_sticky_posts' => !get_theme_mod('enable_sticky_loop_homepage2', true),
      );

      if ( get_theme_mod('category_loop_homepage2') ):
        $args['category_name'] = get_theme_mod('category_loop_homepage2');
      endif;
      $wp_query_homepage2 = new WP_Query( $args );
      // var_dump($wp_query_homepage);
    ?>
    <!-- Row -->
    <div class="row">
    <?php
			while ( $wp_query_homepage2->have_posts() ) : $wp_query_homepage2->the_post();
      /* if we change content to experct, we need to delete this counter */
      hfsi_setPostViews(get_the_ID());
      ?>
      ...
      <article id="post-<?php the_ID(); ?>" class="col-md-4 d-flex align-items-stretch">
        ...
        $classes = array(
          ...
          'mb-4',
          'box-shadow',
          get_theme_mod('bgcolor_homepage2')
        );
        post_class( $classes );
        /* @todo Maybe adding visual effect to sticky elements */
        ?>>
        <?php if ( get_theme_mod('enable_picture_homepage2') ): ?>
        <?php if (has_post_thumbnail( get_the_ID() )): ?>
          <img class="card-img-top" alt="<?php the_title(); ?>" style="height: 225px; width: 100%; display: block;" src="<?= wp_get_attachment_image_src( get_post_thumbnail_id(), 'large')[0] ?>" data-holder-rendered="true">
        <?php else : ?>
        <div class="d-flex flex-row" style="background-color:var(--<?php $color = array('success', 'danger', 'warning', 'secondary', 'dark'); echo $color[rand(0, 4)]; ?>);height: 225px; width: 100%; display: block;">
